            ...
            <div className="flex items-center gap-3 mb-4">
              {seasonNum > 1 && (
                <Link
                  ...
                  aria-label={`Previous season (Season ${seasonNum - 1})`}
                >
                  ←
                </Link>
              )}
              ...
              {nextSeasonNum != null && (
                <Link
                  ...
                  className="flex h-10 w-10 items-center justify-center rounded-lg border border-gray-800 bg-gray-900 text-xl text-gray-300 hover:border-blue-500 hover:text-white transition-colors"
                  aria-label={`Next season (Season ${nextSeasonNum})`}
                >
                  →
                </Link>
              )}
            ...
            <div className="flex flex-wrap items-center gap-3 mb-6">
              {showDetails.releaseDate && (
                <span className="text-sm font-bold text-gray-300">
                  {showDetails.releaseDate.split("-")[0]}
                </span>
              )}
              {showDetails.runtime && (
                <>
              ...
              {showDetails.genres?.length > 0 && (
                <>
                  ...
                  {showDetails.genres.map((genre: string) => (
                    <span
                      key={genre}
                      className="text-xs font-bold uppercase tracking-wider text-blue-300 bg-blue-900/30 px-3 py-1.5 rounded-md border border-blue-800/50"
                    >
                      {genre}
                    </span>
                  ))}
