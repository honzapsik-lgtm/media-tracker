...
export default function SyncLoader({ mediaId }: { mediaId: string }) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data.isSynced) {
            ...
            // Refresh the server component to load the newly constructed timeline!
            router.refresh();
          }
        }
      } catch (err) {
  ...
  return (
    <div className="w-full bg-gray-900 border border-blue-900/50 rounded-2xl p-6 mb-8 animate-pulse shadow-[0_0_15px_rgba(59,130,246,0.15)] flex flex-col sm:flex-row items-center gap-6">
      ...
      <div className="flex-1 space-y-2 text-center sm:text-left">
        <h3 className="text-xl font-bold text-blue-400">Constructing Narrative Timeline...</h3>
        <p className="text-gray-400 text-sm">
          We are currently exploring the deep AniList graph to uncover all sequels, prequels, and spin-offs for this franchise. The timeline will appear here automatically in just a moment!
        </p>
      </div>
    </div>
  );
